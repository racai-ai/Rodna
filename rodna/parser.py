...
class RoBERTHeadFinder(nn.Module):
    ...
    def __init__(self, msd_size: int, head_window_size: int, name_or_path: str = ''):
...
class DPDataset(Dataset):
    ...
    so that they can form batches for LSTMs. Shuffles the lengths so that
...
class RoDepParser(object):
    ...
    def __init__(self, msd_obj: MSD) -> None:
        ...
        self._headmodel = RoBERTHeadFinder(
            ...
            head_window_size=RoDepParser._conf_head_window)

    def read_parsed_file(self, file: str) -> list:
        ...
        with open(file, mode='r', encoding='utf-8') as f:
            for line in f:
                ...
                if len(parts) != 6:
                    ...
                    current_sentence.append((parts[1], parts[3], int(parts[4]), parts[5]))
                # end if
            # end all lines
        # end with
    ...
    def _process_sentence(self, sentence: list, runtime: bool = False) -> Tuple[Union[Tensor, None]]:
        ...
        for i, (word, msd, head, deprel) in enumerate(sentence):
            ...
            if not runtime:
                ...
                center_index = RoDepParser._conf_head_window
                ...
                if head == 0:
                    ...
                    # 1 on the 0 index
                    if head_index < 0 or \
                        head_index >= 2 * RoDepParser._conf_head_window + 1:
                        print(stack()[
        ...
        msd_input_tensor = torch.cat(
            msd_input_tensor, dim=0).view(1, len(tokens), -1).to(_device)

        if not runtime:
            # Shape of (1, len(sentence), feature_size)
            head_output_tensor = torch.tensor(
                head_output_tensor, dtype=torch.long).view(1, -1).to(_device)
        # end if
    ...
    def train(self, train_sentences: list, dev_sentences: list, test_sentences: list):
        ...
        self._lr_scheduler = ExponentialLR(
            optimizer=self._optimizer, gamma=RoDepParser._conf_gamma_lr, verbose=True)
    ...
    def parse_sentence(self, sentence: list) -> List[Tuple]:
        ...
        for i in range(len(parser_sentence)):
            ...
            for k in range(prob_heads_for_i.shape[0]):
                ...
                off = k - RoDepParser._conf_head_window
                j = i + off
    ...
    def do_uas_and_las_eval(self, sentences: list, desc: str):
        """Does the Unlabeled Attachment Score calculation, given a dev/test set."""
        ...
        print(f'UAS = {uas:.5f}, LAS = {las:.5f} on {desc}set', file=sys.stderr, flush=True)


if __name__ == '__main__':
    mso = MSD()
    par = RoDepParser(msd_obj=mso)
    ...
    training = par.read_parsed_file(training_file)
    ...
    development = par.read_parsed_file(development_file)
